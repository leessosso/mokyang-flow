...
- **가족·가족원**: 임원은 해마다 상반기에 목사가 정하고 하반기까지 둡니다. 가족과 가장은 상반기·하반기마다 다시 짭니다. 가장·임원으로 앉힐 사람이 없으면 그 자리에서 로그인 계정을 추가합니다. 가족원은 임원·목사가 「성도 명단」에서 한 명씩 넣거나, 이름 목록·CSV·엑셀로 한꺼번에 넣습니다. 가족 화면에서는 명단을 보고, 이미 있는 가족원을 다른 가족으로 옮깁니다. 다음 해 상반기를 열면 임원 직책은 끝나고, 하반기를 열면 올해 임원은 유지됩니다. 이전 가족원은 `가족`에서 미배정으로 보여 새 가족으로 옮깁니다.
