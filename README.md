...
- **가족·가족원**: 1년에 상반기·하반기 두 번 구성합니다. 임원·목사가 그 학기 가족을 만들고 가족원을 배정하며, 가장은 `내 가족`에서 가족원을 추가할 수 있습니다. `가장·임원 관리`에서 다음 학기를 열면 이전 가족은 남겨 두고 새로 짭니다.
...
- **출석**(`/attendance`): 임원·목사가 주일을 열면, 가장이 자기 가족원마다 1-3부/4부 참석·방송과 4부 이후 가족모임 참석 여부를 체크합니다. QR은 교회 출입 명단(CSV/xlsx)을 임원·목사가 올려 자동으로 켜지고, 동명이인·미매칭은 그 화면에서 직접 켤 수 있습니다.
- **참여조사**(`/surveys`): 식수 조사 같은 일회성 행사 참여 인원 조사를 임원·목사가 만들고(예/아니오·인원·메모 질문 조합), 가장이 가족원을 대신해 응답합니다. 임원·목사는 가족별 응답률과 합계를 봅니다.
