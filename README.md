...
- **출석**(`/attendance`): 매주 주일 출석은 일정을 등록하지 않아도 항상 열려 있습니다. 가장이 자기 가족원마다 1-3부/4부 참석·방송과 4부 이후 가족모임 참석 여부를 체크합니다. QR은 교회 출입 명단(CSV/xlsx)을 임원·목사가 올려 자동으로 켜지고, 동명이인·미매칭은 그 화면에서 직접 켤 수 있습니다. 수련회처럼 매주가 아닌 날은 임원·목사가 **비정기 출석체크**를 열어 참석만 받습니다.
